...
const EmployeeDashboard = () => {
  ...
  return (
    <div style={{ padding: '2rem 0' }}>
      ...
      <div className="flex-cards" style={{ marginBottom: '2rem' }}>
        <div className="card">
          <p style={{ fontSize: '0.8rem', color: '#888', margin: 0 }}>Attendance (Total)</p>
          <h3 style={{ fontSize: '1.8rem', fontWeight: 700, color: '#1a237e', margin: '0.3rem 0 0' }}>{attendance.length}</h3>
        </div>
        <div className="card">
          <p style={{ fontSize: '0.8rem', color: '#888', margin: 0 }}>Present Days</p>
          <h3 style={{ fontSize: '1.8rem', fontWeight: 700, color: '#2e7d32', margin: '0.3rem 0 0' }}>{presentCount}</h3>
        </div>
        <div className="card">
          <p style={{ fontSize: '0.8rem', color: '#888', margin: 0 }}>Pending Leaves</p>
          <h3 style={{ fontSize: '1.8rem', fontWeight: 700, color: '#e65100', margin: '0.3rem 0 0' }}>{pendingLeaves}</h3>
        </div>
        <div className="card">
          <p style={{ fontSize: '0.8rem', color: '#888', margin: 0 }}>Last Salary</p>
          <h3 style={{ fontSize: '1.8rem', fontWeight: 700, color: '#1565c0', margin: '0.3rem 0 0' }}>₹{lastPayroll?.netSalary?.toLocaleString() || '0'}</h3>
        </div>
      </div>
                ...
                <tbody>
                  {attendance.slice(-5).reverse().map((a, i) => (
                    <tr key={a._id} style={{ background: i % 2 === 0 ? '#fafafa' : '#fff' }}>
                      ...
                      <td style={tdStyle}><span style={badge(a.status)}>{a.status}</span></td>
                    </tr>
                  ))}
                ...
                <tbody>
                  {leaves.slice(-5).reverse().map((l, i) => (
                    <tr key={l._id} style={{ background: i % 2 === 0 ? '#fafafa' : '#fff' }}>
                      ...
                      <td style={tdStyle}><span style={badge(l.status)}>{l.status}</span></td>
                    </tr>
                  ))}
...
const thStyle = { padding: '0.75rem', textAlign: 'left', borderBottom: '2px solid #eee', fontSize: '0.75rem', color: '#888', fontWeight: 600, textTransform: 'uppercase', whiteSpace: 'nowrap' };
const tdStyle = { padding: '0.75rem', borderBottom: '1px solid #f0f0f0', fontSize: '0.85rem', whiteSpace: 'nowrap' };
const badge = (s) => ({
